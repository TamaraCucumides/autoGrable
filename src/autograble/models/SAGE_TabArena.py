"""
Heterogeneous GraphSAGE baseline for AutoGrable -- general tabular-arena variant.

Same architecture as SAGE_Fraud.py, but tuned for generic tabular
classification tasks rather than fraud detection specifically:
  ...
  4. Binary target: single logit, plain BCEWithLogitsLoss (pos_weight defaults
     to 1.0 -- no automatic reweighting for class imbalance). Model selection
     on a validation metric, AUROC by default (see `SAGEConfig.selection_metric`).
     If a task turns out to be heavily imbalanced, either set `pos_weight`
     explicitly or reach for SAGE_Fraud.py instead.

Works with either:
  - a unified transductive HeteroData with train/val/test masks on "row",
  - or three per-split graphs (inductive), by passing them separately.
"""
...
class SAGEConfig:
    ...
    epochs: int = 300
    patience: int = 40           # early stopping on val AUROC
    grad_clip: float = 1.0

    pos_weight: float = 1.0      # plain BCE by default -- not imbalance-aware

    selection_metric: str = "auroc"  # "auroc" | "ap" -- metric used for early stopping/model selection
...
def train_model(
    ...
    loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(cfg.pos_weight, device=device))
